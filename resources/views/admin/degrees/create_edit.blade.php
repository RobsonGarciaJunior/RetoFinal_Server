                 ...
                 <div class="form-group mb-3">
                     ...
                     <select class="form-select" name="department_id" id="department_id" required>
                         @foreach ($departments as $department)
                             <option value="{{ $department->id }}" @if (isset($degree) && $degree->department->name == $department->name) selected @endif>
                                 {{ $department->name }}
                             </option>
                         @endforeach
                     </select>
                                         ...
                                         <div class="card-body">
                                             <label class="checkbox-inline">
                                                 <input type="checkbox" name="modules[]" id="modules[]"
                                                     value="{{ $module->id }}"
                                                     @if (isset($degree) && $degree->modules->contains('name', $module->name)) checked @endif>
                                                 {{ $module->name }}
                                             </label>
                                         </div>
                                     </div>
