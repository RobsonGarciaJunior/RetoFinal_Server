        ...
        <nav class="navbar navbar-expand-md navbar-light shadow-sm">
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <!-- Left Side Of Navbar -->
                    @if( !Auth::user()==null && !auth()->user()->roles->contains(3))
                        <ul class="navbar-nav me-auto">
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('degrees.index') }}">{{trans('app.degrees') }}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('departments.index') }}">{{trans('app.departments') }}</a>
                            </li>
                            <li class="nav-item">
                    ...
                    <ul class="navbar-nav ms-auto">
                        <div class="dropdown">
                            <button class="btn dropdown-toggle text-white" type="button" id="languageDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-translate"></i>
                                  {{trans('app.language') }}
                            </button>
                            <ul class="dropdown-menu dropdown-menu-bottom" aria-labelledby="languageDropdown">
                                <li><a class="dropdown-item" href="{{ route('language', 'es') }}">{{trans('app.spanish') }}</a></li>
                                <li><a class="dropdown-item" href="{{ route('language', 'en') }}">{{trans('app.english') }}</a></li>
                            </ul>
                        </div>
